...
def service(action=None, name=None, user=None, options="", create_pid_dir=False,
  ...
  if name == "nfs3" :
    pid_file = format("{hadoop_pid_dir_prefix}/privileged-root-nfs3.pid")
    custom_export = {
  ...
  if params.security_enabled and name == "datanode":
    ...
    hadoop_secure_dn_pid_file = params.datanode_pid_file
    process_id_exists_command = as_sudo(["test", "-f", hadoop_secure_dn_pid_file]) + " && " + as_sudo(["pgrep", "-F", hadoop_secure_dn_pid_file])
