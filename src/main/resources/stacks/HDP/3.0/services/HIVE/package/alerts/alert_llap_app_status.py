...
def execute(configurations={}, parameters={}, host_name=None):
  ...
  try:
    ...
    if STACK_ROOT in configurations:
      llap_status_cmd = configurations[STACK_ROOT] + format("/current/hive-server2/bin/hive --service llapstatus --name {llap_app_name}  --findAppTimeout {LLAP_APP_STATUS_CMD_TIMEOUT}")
    else:
