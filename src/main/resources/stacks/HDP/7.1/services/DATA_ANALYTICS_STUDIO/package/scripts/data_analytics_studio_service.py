...
def create_credential_providers(action):
  import params
  ...
  if action == 'start':
    # create the database jceks file in hdfs also so that the das_hive_query table can use it.
    targetFilePath = format("{default_fs}/{das_db_password_jceks_hdfs_location}")
    sourceFilePath = os.path.join(params.conf_dir, 'data_analytics_studio-database.jceks')
    params.HdfsResource(targetFilePath,
...
def data_analytics_studio_webapp_action(action):
  ...
  if action == "start":
    create_credential_providers(action)
    Execute(cmd,
...
def data_analytics_studio_event_processor_action(action):
  ...
  if action == "start":
    ...
    Execute(cmd,
            ...
            environment = env,
            user = params.data_analytics_studio_user)
  elif action == 'schema-tool':
    create_credential_providers(action)
    cmd += format(" {das_sysdb_tables_hql}")
    Execute(cmd,
            environment = env,
            user = params.data_analytics_studio_user)
  else:
...
def data_analytics_studio_initdb():
    import params
    create_credential_providers('install')
